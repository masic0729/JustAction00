...
public class Player : Character
{
    ...
    protected override void Init()
    {
        ...
        // 기존: hp = 100;
        // 변경: base.Init()에서 CharacterStatData 기반으로 AttributeSet이 초기화되므로 제거한다
        // 만약 플레이어의 초기 체력을 ScriptableObject가 아닌 코드로 강제 지정하고 싶다면
        // GetAttributeSet().ApplyHealthChange(원하는값 - GetHp(), null) 형태로 사용할 것
    ...
    /// 기존: statDatas[(int)AddStatName.LevelUp] 직접 접근
    /// 변경: ModifierContainer의 LevelUp 소스 보정치를 읽어 수정 후 재등록한다
    /// StatModifierData는 struct이므로 반드시 로컬에 받아서 수정 후 재설정한다
    /// </summary>
    public void LevelUpForStatUp()
    {
        float statUpValue = 5f;

        // LevelUp 소스의 현재 보정치를 가져와 수치를 누적한다
        ...
        // 레벨업 시 현재 체력도 최대 체력 상승분만큼 함께 올린다
        // 기존: SetHp(GetHp() + statUpValue)
        // 변경: AttributeSet의 ApplyHealthChange를 통해 체력을 조정한다
