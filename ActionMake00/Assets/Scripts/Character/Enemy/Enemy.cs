...
public class Enemy : Character
{
    ...
    public bool isDefault = false;                               //몬스터가 본인 구역을 벗어날 때, 비헤이비어 트리 상에서 자동으로 복귀하기 위한 용도. 활성화 시 복귀한다
    ...
    void CheckCharacterActivityZone()
    {
        ...
        {
            isDefault = true;  // 기존 false → true로 수정. 범위 이탈 시 복귀 모드로 전환
        }
    }
    ...
    void RotateByAttack()
    {
        // 기존: this.hp <= 0 으로 직접 필드 접근
        // 변경: GetIsDead()로 사망 여부를 확인한다. hp 필드는 AttributeSet 내부로 이동됨
    ...
    {
        // 기존: moveSpeed 직접 접근
        // 변경: GetMoveSpeed()로 접근한다. moveSpeed 필드는 AttributeSet 내부로 이동됨
